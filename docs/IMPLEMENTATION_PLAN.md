# PraxLight â€” Implementation Plan (20 phases)

Source: the "PHASE 1 â†’ PHASE 20 MASTER IMPLEMENTATION PROMPT" supplied for
...
Status values: `NOT STARTED` Â· `IN PROGRESS` Â· `COMPLETE` Â· `BLOCKED`

| Phase | Title | Status | Notes |
|---|---|---|---|
| 1 | Baseline + architecture hardening | **COMPLETE** | This audit. See `docs/HANDOFF_PHASE_1.md`. No code changed â€” audit only, per the phase's own acceptance criteria. |
...
| 5 | Perception fusion engine | NOT STARTED | Depends on Phases 3â€“4 existing first; nothing to fuse yet beyond DOM+regex, which already happens inline in `detectors.js`. |
...
| 8 | Privacy policy engine 2.0 | NOT STARTED | Current `policy-engine.js` is a flat severityâ†’action map, no per-domain policy, no confidence threshold config. |
...
| 13 | Observe â†’ verify â†’ replan loop | NOT STARTED | Current execution treats a successful DOM `.click()` as done; no post-action state verification. |
| 14 | Human approval + risk engine | PARTIAL | Approval exists and is policy-driven for `risk=="high"`, but still falls back to reason-text keyword matching as a second trigger â€” the plan asks for this to move to explicit action risk metadata only. |
...
| 19 | Security + privacy attack testing | NOT STARTED | No adversarial test suite (prompt injection, forged manifests, malformed hashes) exists yet beyond the "missing manifest â†’ 400" happy-path-adjacent test. |
...
- Phases 3 (OCR) and 4 (vision) are independent of each other but Phase 5
  (fusion) needs both to exist to be meaningful â€” don't build fusion against
...
- Phase 9's payload hash is a natural follow-on to Phase 1's baseline since
  ...
  `server/main.py`) already covered in this audit â€” a reasonable Phase 2
...
- Phase 10 (real model reasoning) should keep the deterministic fallback
  intact â€” the plan is explicit about this, and it's also what makes the
...
- Phase 15 (instrumentation) will be much cheaper to do accurately if it's
  done before Phases 3/4/5 add OCR and vision latency to measure â€” consider
