# PraxLight â€” SIH26171 Alignment

## Official requirement mapping

| SIH26171 requirement | Implementation | Status |
|---|---|---|
| Lightweight visual perception runs locally | DOM + text perception (`extension/content/perception.js`) | Done â€” text/DOM only, no vision/OCR model yet |
...
| Sanitized context may reach a server-side reasoning model | `background.js` gate â†’ `POST /api/agent/act` | Done |
...
| Visual-context accuracy | N/A yet â€” no vision model | Not measured (honestly, not "0%" â€” just not built) |
| Sensitive/PII detection precision & recall | Node unit tests cover the rules layer's behavior; no labeled benchmark dataset built yet | Partially done â€” unit-tested, not yet benchmarked against a labeled set |
...
| End-to-end latency | Network Guard log records real `latencyMs` per request | Partially done â€” captured per-request, not aggregated/reported |
...
| OSSRedact | Local privacy gateway redacting before a cloud LLM call, rehydrating on the reply | Closest in spirit to PraxSight's privacy gate. Still text-in/text-out around a chat call â€” no browser action execution, no validator, no approval gate. |
| Chrome Built-in AI / Gemini Nano | On-device model PraxSight is designed to build on top of (Phase 3), not compete with | â€” |
...
- Full Firefox parity (only "the fallback path doesn't crash it" is in
  scope for a future pass â€” not built yet at all in this one)
- The five-way ablation study from the original 56-phase brief
- The four-page demo suite (banking/HR/login/support) â€” one page,
