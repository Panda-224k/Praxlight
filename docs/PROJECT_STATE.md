...
- Phase 1 delivered as `PraxSlight-phase1-updates.zip` (5 restored docs + 5
  new project-state docs) for the user to commit — this audit assumes that
...
| jsdom | 30.0.1 (dev/test only — **new in Phase 2**, declared in `package.json`, needed only to run `tests/test_perception.cjs`) |
...
loading the extension in Chrome — see `TEST_STATUS.md`.
...
materialization time — anywhere from 2% to 88% of their intended content
...
`CURRENT_IMPLEMENTATION.md`) were unaffected — that's not a coincidence,
...
## Phase 2 update: DOM perception hardening — COMPLETE

`extension/content/perception.js` was extended (not replaced — every field
...
- `formId` — nearest containing `<form>`, resolved via the native `.form`
  property so `form="..."` attribute associations work, not just DOM nesting
- `inViewport` — deliberately distinct from the existing CSS-visibility
...
- same-origin iframe perception, one level deep (`extractIframes`) —
...
- open shadow-root traversal (`collectAllRoots`) for inputs, interactive
  elements, and text — closed shadow roots remain genuinely invisible to
...
- a real bugfix: `isVisible` now resolves computed style via the element's
  *own* window (`el.ownerDocument.defaultView`) rather than always the
  top-level window — matters once same-origin iframe content is perceived

**New test file**: `tests/test_perception.cjs`, 13 tests, using jsdom
(declared in the new root `package.json` as a dev-only dependency — the
...
pass unchanged — confirms the additive schema changes didn't disturb
...
sees — it still whitelists a fixed field set. An agent that could see
...
client residual scan → background manifest+residual gate → server manifest
check. Confirmed working end to end in this audit (see `TEST_STATUS.md`).

## Working features (verified this audit, not assumed)

- DOM/text perception producing plain-data objects (`perception.js`) —
  **as of Phase 2, this is now covered by 13 passing jsdom-backed unit
  ...
  a real browser (jsdom is not a browser) — see `TEST_STATUS.md`.
- Rules-based detection: structural signals + regex + Luhn-checked card
  numbers (`detectors.js`) — 11/11 unit tests pass against the actual
  repository file.
- Semantic redaction with counted vs. counterless tokens (`redaction.js`) —
  covered by the same test file, passing.
- Policy engine defaulting unrecognized severities to `redact`
  (`policy-engine.js`) — covered by a dedicated fail-closed test, passing.
- Structured action schema + validator: unknown `psId` rejected, `navigate`
  ...
  irreversible-sounding reasons force `requires_approval` — 6/6 pytest
...
- FastAPI backend: `/api/health`, `/demo/*` static mount, and
  ...
  `TEST_STATUS.md` for the exact request/response pairs) — a valid
...
- No screenshot capture, OCR, or vision model anywhere in the codebase —
...
- `model-backends.js`'s `detect()` returns `[]` unconditionally for both the
  Gemini Nano and Transformers.js adapters — real interfaces, no live model.
- `server/agent.py` is a deterministic keyword matcher against interactive
  ...
  its own docstring and in `CURRENT_IMPLEMENTATION.md` — no mislabeling
  found in this audit.
...
- No independent (out-of-extension) transmission proof — the Network Guard
  ...
  audit.
- Firefox: untested, not just "unlikely to work" — genuinely never opened
...
overhead — this is a lower bound, not a real end-to-end network latency
number):
...
Real browser-extension → localhost network latency was not separately
measured in this audit due to sandbox networking constraints (see
`TEST_STATUS.md`). Do not quote the numbers above as "network latency" —
